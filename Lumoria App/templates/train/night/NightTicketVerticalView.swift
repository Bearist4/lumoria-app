//
//  NightTicketVerticalView.swift
//  Lumoria App
//
//  Vertical "Night" train ticket. Same palette and structure as the
//  horizontal variant, restacked for portrait — main content on top,
//  summary stub below the dashed divider.
//
//  Design: figma.com/design/1vAhPHcA6A3SRsQJkkylIU/Tickets?node-id=8-1527
//

import SwiftUI

struct NightTicketVerticalView: View {
    ...
    /// Fraction of the total height occupied by the bottom stub.
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            ZStack(alignment: .topLeading) {
                ...
                VStack(spacing: 0) {
                    mainSection(scale: s)
                        .frame(height: h * (1 - stubFraction))
                    
                    stubSection(scale: s)
                        .frame(height: h * stubFraction)
                }
                .frame(width: w, height: h)
            }
            .frame(width: w, height: h)
...
#Preview("Night — vertical") {
    NightTicketVerticalView(ticket: NightTicket(
        company: "Company",
